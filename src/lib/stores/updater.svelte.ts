...
export type UpdaterState = 'checking' | 'up-to-date' | 'available' | 'downloading' | 'ready' | 'error'
...
class UpdaterStore {
  ...
  async init() {
    ...
    try { this.currentVersion = await getVersion() } catch { /* ignore */ }
    ...
    try {
      await this.#handle.downloadAndInstall((event) => {
        if (event.event === 'Started') {
          ...
          this.downloadPct = this.totalMB > 0
            ? Math.min(99, Math.round((this.downloadedMB / this.totalMB) * 100))
            : 0
