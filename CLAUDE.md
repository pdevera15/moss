...
Editor            CodeMirror 6
Database          SQLite via tauri-plugin-sql (runs in Rust, not JS)
...
6. SQLite operations live in Rust commands, not JavaScript
   JS calls invoke('command_name'), Rust does the actual SQL.

7. Tailwind v4 — @import "tailwindcss" in app.css, that's it
   No tailwind.config.js, no npx tailwindcss init needed.

8. SvelteKit MUST stay in SPA mode
...
❌ Putting SQL logic in TypeScript — it belongs in Rust commands
