...
class TasksStore {
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      for (const s of subRows) {
        ...
        subsByParent
          .get(pid)!
          .push({ id: s.id, title: s.title, done: Boolean(s.done) });
      }
  ...
  async toggleTask(id: string): Promise<void> {
    ...
    try {
      ...
      await db
        .update(tasks)
        .set({ done: task.done, updated_at: Date.now() })
  ...
  async toggleSubtask(taskId: string, subId: string): Promise<void> {
    ...
    try {
      ...
      await db
        .update(tasks)
        .set({ done: sub.done, updated_at: now })
        .where(eq(tasks.id, subId));
      await db
        .update(tasks)
        .set({ done: task.done, updated_at: now })
  ...
  async updateDueDate(id: string, dueDate: number | null): Promise<void> {
    ...
    try {
      ...
      await db
        .update(tasks)
        .set({ due_date: dueDate, updated_at: Date.now() })
        .where(eq(tasks.id, id));
    } catch (err) {
