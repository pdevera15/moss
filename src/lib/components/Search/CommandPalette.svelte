<script lang="ts">
  ...
  let tagFilters = $derived(
    tags
      .slice(0, 3)
      .map((t) => ({ icon: "🔍", label: `Search in ${t}`, tag: t })),
  );
  ...
  let results = $derived(
    !isComposing && query.trim() ? ftsResults.slice(0, 8) : [],
  );
  ...
  function handleKeydown(e: KeyboardEvent) {
    ...
    if (e.key === "ArrowDown") {
      ...
      requestAnimationFrame(() =>
        document
          .querySelector('.result-row[data-active="true"]')
        ...
        {:else}
          <div class="results">
              ...
              >
                <div class="result-head">
                  <span class="result-title"
                    >{@html hl(result.title || "Untitled", query)}</span
                  >
                  <span class="result-date"
                    >{formatDate(result.updated_at)}</span
                  >
                </div>
                {#if preview}
            ...
            {#if semanticResults.length > 0}
              <div class="section-label" style="padding-top: 8px;">
                Similar notes
              </div>
              {#each semanticResults as result (result.id)}
                ...
                <div
                  ...
                  onkeydown={(e) =>
                    e.key === "Enter" && handleSemanticSelect(result)}
                >
                  <div class="result-head">
                    <span class="result-title"
                      >{result.title || "Untitled"}</span
                    >
                    <span class="sem-score"
