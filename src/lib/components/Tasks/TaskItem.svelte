<script lang="ts">
  ...
  function commitSubTitle() {
    ...
    if (trimmed && sub && trimmed !== sub.title)
      onupdatesub(task.id, id, trimmed);
  }
...
<div class="swipe-container" class:is-done={task.done}>
  ...
  <div
    class="delete-zone"
    style:opacity={Math.min(1, -swipeX / DELETE_THRESHOLD)}
  >
    <svg
      width="14"
      height="14"
  ...
  >
    ...
    <div class="task-content">
      ...
      <div class="title-row" class:has-subs={hasSubs && expanded}>
        ...
        {:else}
          <span
            ...
            onpointerdown={(e) => e.stopPropagation()}>{task.title}</span
          >
        {/if}
        ...
        {#if task.dueDate !== null}
          <span class="due-chip-wrap">
            <button
              ...
              onpointerdown={(e) => e.stopPropagation()}
              aria-label="Edit due date">{dueDateLabel}</button
            >
            <button
              class="due-clear"
              onclick={(e) => {
              ...
              onpointerdown={(e) => e.stopPropagation()}
              aria-label="Clear due date">×</button
            >
          </span>
        {:else if !task.done}
          <button
        ...
        {#if hasSubs}
          <div class="progress-pill">
            ...
            <span class="progress-count" class:complete={allSubsDone}
              >{doneSubs}/{task.subtasks.length}</span
            >
          </div>
          ...
          <button
            ...
            aria-label="Toggle subtasks">▾</button
          >
        {/if}
      </div>
              ...
              {:else}
                <span
                  ...
                  onkeydown={(e) =>
                    e.key === "Enter" &&
                    !sub.done &&
                    startEditSub(sub.id, sub.title)}
                  onpointerdown={(e) => e.stopPropagation()}>{sub.title}</span
                >
              {/if}
            </div>
          {/each}
      ...
      {#if !task.done}
        ...
        {:else}
          <button class="add-sub-btn" onclick={() => (addingSubtask = true)}
            >+ subtask</button
          >
        {/if}
      {/if}
...
<style>
  ...
  .task-row.rolled {
    ...
    border-color: rgba(196, 168, 74, 0.18);
  }
